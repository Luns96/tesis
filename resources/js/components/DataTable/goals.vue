  ...
  <v-card>
    <v-btn
      ...
      <v-text-field
            ...
            label="Cantida de los metas"
            prepend-icon="fas fa-sort-numeric-up"
            readonly
        ></v-text-field>
         <v-text-field
            v-model="task"
            label="Metas completo"
            prepend-icon="fas fa-sort-numeric-up"
            readonly
        ></v-text-field>
    </v-layout>
      <v-layout row col>
    <template v-if="this.objective.estatud === 1">
      <v-text-field
            v-model="P"
            label="Estatus"
    ...
    <template v-else>
      <v-text-field
        ...
        <v-text-field
            v-model="viewCA"
            label="Fecha de creacion de la objectivo"
...
<script>
  export default {
    ...
    methods:{
        ...
        getobjective () {
          var url= "/api/objective/info/" + this.id;
            axios.get(url).then(response=>{
                this.objective = response.data;
                this.viewname = this.objective.name;
                this.viewquantity = this.objective.quantity;
                this.viewCA = this.objective.created_at;
            });
        },

        getGoal(){
            var url= "/api/goal/index/" + this.id;
            axios.get(url).then(response=>{
